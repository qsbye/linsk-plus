...
// writeEmbeddedFile writes embedded bytes to the target path with hash validation.
func (s *Storage) writeEmbeddedFile(data []byte, hash []byte, out string) error {
	outClean := filepath.Clean(out)
	...
	s.logger.Info("Extracting embedded file", "to", outClean, "size", humanize.Bytes(uint64(len(data))))

	// Validate hash before writing if provided.
	if hash != nil {
		h := sha256.New()
		h.Write(data)
		sum := h.Sum(nil)
		if !bytes.Equal(sum, hash) {
			return fmt.Errorf("embedded file hash mismatch: want '%v', have '%v'", hex.EncodeToString(hash), hex.EncodeToString(sum))
		}
	}

	// Write with progress reporting.
	blockSize := 1024 * 1024 // 1MB blocks for embedded files
	totalSize := len(data)
	var written int
	...
	for written < totalSize {
		...
		if err != nil {
			return errors.Wrap(err, "write embedded data")
		}
		written += n

		percent := float64(written) / float64(totalSize)
		s.logger.Info("Extracting embedded file", "out", outClean, "done", humanize.Bytes(uint64(written)),
			"percent", math.Round(percent*100*100)/100)
	}

	s.logger.Info("Successfully extracted embedded file", "to", outClean, "out-size", humanize.Bytes(uint64(written)))
