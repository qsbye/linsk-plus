...
import (
	...
	"github.com/AlexSSD7/linsk/assets"
...
func (s *Storage) CheckDownloadBaseImage(ctx context.Context) (string, error) {
	...
	if err != nil {
		...
		// Image doesn't exist. Extract from embedded resources.
		embeddedData := assets.GetAlpineBaseImageBytes(runtime.GOARCH)
		if embeddedData == nil {
			return "", fmt.Errorf("no embedded Alpine base image available for architecture %s", runtime.GOARCH)
		}

		err := s.writeEmbeddedFile(embeddedData, constants.GetAlpineBaseImageHash(), baseImagePath)
		if err != nil {
			return "", errors.Wrap(err, "extract embedded base alpine image")
		}
...
func (s *Storage) RunCLIImageBuild(showBuilderVMDisplay bool, overwrite bool) int {
	...
	if err != nil {
		slog.Error("Failed to check or extract base VM image", "error", err.Error())
		return 1
	}

	biosPath, err := s.CheckDownloadVMBIOS(context.Background())
	if err != nil {
		slog.Error("Failed to check or extract VM BIOS", "error", err.Error())
		return 1
	}
...
func (s *Storage) CheckDownloadAarch64EFIImage(ctx context.Context) (string, error) {
	_ = ctx // ctx is no longer needed for embedded resources, but kept for API compatibility.
	...
	if err != nil {
		...
		// EFI image doesn't exist. Extract from embedded resources.
		embeddedData := assets.GetAarch64EFIImageBytes()
		if embeddedData == nil {
			return "", fmt.Errorf("no embedded aarch64 EFI image available")
		}

		err := s.writeEmbeddedFile(embeddedData, constants.GetAarch64EFIImageHash(), efiImagePath)
		if err != nil {
			return "", errors.Wrap(err, "extract embedded aarch64 efi image")
		}
